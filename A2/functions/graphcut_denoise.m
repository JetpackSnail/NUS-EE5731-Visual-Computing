...
patch_gray = rgb2gray(patch);
        ...
        diagVec1 = sparse(repmat([ones(w-1, 1); 0], h, 1));     % Make the first diagonal vector (for horizontal connections)
        diagVec1 = diagVec1(1:end-1);                           % Remove the last value
        diagVec2 = sparse(ones(w*(h-1), 1));                    % Make the second diagonal vector (for vertical connections) 
